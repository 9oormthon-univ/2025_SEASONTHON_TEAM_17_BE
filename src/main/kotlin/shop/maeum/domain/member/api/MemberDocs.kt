...
interface MemberDocs {
    ...
    @Operation(
        summary = "닉네임 변경",
        description = "토큰을 기반으로 회원의 닉네임을 변경합니다."
    )
    @ApiResponse(
        responseCode = "200",
    ...
    @ApiResponse(
        responseCode = "400",
        description = "닉네임 변경 실패 (중복 또는 유효하지 않은 회원)",
        content = [Content(
            mediaType = "application/json",
            schema = Schema(implementation = RspTemplate::class),
            examples = [ExampleObject(
                name = "닉네임 변경 실패 응답 예시",
                value = """
                {
                    "statusCode": 400,
    ...
    fun changeNickname(changeNicknameReqDto: ChangeNicknameReqDto): RspTemplate<ChangeNicknameResDto>
}
